...
/** Preview path inside admin (vite base /admin/). Stored value stays storefront path /jobs/... */
export function adminJobImageSrc(src) {
  const path = src || JOB_IMAGE_PRESETS[0].src;
...
function normalizeJob(job) {
  ...
  return {
    ...
    image: String(job.image || "").trim() || JOB_IMAGE_PRESETS[0].src,
    summary:
